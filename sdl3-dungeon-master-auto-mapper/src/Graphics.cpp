...
//  while (d.Word23228!=0) {};
  ...
  while (bytesNeeded > 0)
  {
    ...
    memmove(buffer,d.ClusterCache+clusterOffset,(i16)bytesToMove);
...
void ReadAndExpandGraphic(i32 graphicNum, ui8 *dest, i16 P3, i16 P4, i32 maxSize) // TAG022b86
{
  ...
  printf("ReadAndExpandGraphic opening %s \n",buf);
  /*sf = SDL_LoadBMP(buf);
  if (sf) {
      int x,y;
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  if (D0L != d.GraphicDecompressedSizes[D7W])
  {
    ...
    if (D6W == 0)
    {
      memmove( (ui8 *)d.pViewportBMP,dest, d.GraphicDecompressedSizes[D7W]);
    };
  }
  else
  {
