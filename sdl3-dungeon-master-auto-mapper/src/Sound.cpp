...
uint8_t *SoundDecode(uint8_t *pGraphic, int32_t numSample, int32_t volume, uint32_t *sizeOfWavFile)
{ // volume is a divisor
  char *pN;
  FILE *f=NULL;
  unsigned char *pW;
  SNDHEAD *WavBuf;
  int32_t i, n, repeat, sample, nibble;
  bool even;
#ifdef _SNDDEBUG
  static bool snddebug = true;
  if(snddebug) f=fopen("snddbg.txt","w");
  ...
  if (!usingDirectX)
  {
    if (curVol != volume)
    {
      ...
      int32_t i;
      for (i=0; i<128; i++) 
        volTable[i] = (uint8_t)((i-128-volume/2)/volume + 128);
      for (i=128; i<256; i++)
        volTable[i] = (uint8_t)((i-128+volume/2)/volume + 128);
      curVol = volume;
    };
  };
  WavBuf = (SNDHEAD *)UI_malloc(numSample + 58,0);

  *sizeOfWavFile = numSample + 58;
  ...
  if(f)
  {
    for (uint8_t *kk=pGraphic; kk<(uint8_t *)(pN+1); kk+=8)
    {
      fprintf(f,"%08x ",kk-pGraphic);
    ...
    fprintf(f,"Ending pN = %08x\n", (uint32_t)(uintptr_t) pN);
    fclose(f);
    f=NULL;
  };
  ASSERT(n>=0,"n");
  memcpy(WavBuf->byte0,"RIFF",4);
  WavBuf->Size = numSample+58-4;
...
class SOUNDER
{
  char *m_wave[MAXWAVE];
  char *m_toFree;
  uint32_t sizeOfWaveSound[MAXWAVE];
...
public:
  ...
  i32 CheckQueue(void);
  void AddWave(char *wave, uint32_t size , int32_t attenuation);
  void AddWave(char *wave, int32_t attenuation);
...
i32 SOUNDER::CheckQueue()
{
  ...
  if (m_wave[1] != NULL)
  {
    if (UI_PlaySound(m_wave[1], sizeOfWaveSound[1], SOUND_ASYNC|SOUND_MEMORY, m_attenuation[1]))
    {
      ...
      for (i=0; i<MAXWAVE-1; i++)
      {
        ...
        sizeOfWaveSound[i] = sizeOfWaveSound[i+1];
      };
      m_wave[i] = NULL;
    };
    for (i=1; i<MAXWAVE; i++)
...
#if MAXWAVE>1
void SOUNDER::AddWave(char *wave, uint32_t size , int32_t attenuation)
{
  int32_t i;
  if (m_wave[0] == NULL)
  {
    ...
    m_wave[0] = wave;
    sizeOfWaveSound[0] = size;

    m_attenuation[0] = attenuation;
    UI_PlaySound(m_wave[0], size, SOUND_ASYNC|SOUND_MEMORY,m_attenuation[0]);
    return;
  };
  for (i=0; i<MAXWAVE; i++)
  {
...
// Now we want to add the new wave to the queue
  AddWave(wave,size,attenuation); //And play it if is the only one.
...
// *********************************************************
//   TAG001d16
void StartSound(uint8_t *SoundBytes,
                int32_t /*timerData*/,
                int32_t highVolume)
{
  int32_t size, volume;
