...
//loop:
  ...
  if (d.Word23138 == 0)
  {
    ...
    if (d.TextScanlineScrollCount >= 0)
    {
      ...
      memmove(d.LogicalScreenBase+172*160,
	      d.LogicalScreenBase+172*160+160, // Up one line
              D1W);
    ...
    memmove(d.LogicalScreenBase + D2L,
	    (ui8 *)d.newTextLine,   // Bitmap of new text line.
            D1W);
    Invalidate();
    //RestoreRegs(0x030f);
tag000dbe:
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  TAG0088b2((ui8 *)d.SavedCursorPixels,
            ...
            16,
            -1);
  memmove( (ui8 *)d.Pointer23186,(ui8 *)A2, 288);
  ShrinkBLT((ui8 *)A3,
            d.SavedCursorPixels,
            16,
            16,
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  if (d.Word23144 == 0)
  {
    ...
    memmove( (ui8 *)d.Pointer23178,(ui8 *)A2, 256);
    TAG0088b2((ui8 *)d.LogicalScreenBase,
              (ui8 *)d.Pointer23178,
              &d.wRectPos23222,
              rectPosA3->w.x1,
...
tag002b30:
  ...
  D7L &= 0xffff;
  D0 = D7;
  memmove( d.CursorBitmap,(ui8 *)A2, D0W);
  {
    i16 num = sw(D6W<<4);
  ...
  { // Any of the F1 cells.
    ...
    if (relativePosition == 12) //Wall immediately in front of party
    {
      ...
      if (graphicIndex >= 0)
      {
        i32 n;
        graphicID++;
        //memmove( (pnt)d.ViewportObjectButtons[5],A3, 4); // ?????
        memmove( (ui8 *)&d.ViewportObjectButtons[5],(ui8 *)pWDP, 4); // ?????
...
//
    ...
    if (farRightSidewall)
    {
      ...
      MakeMirror(pFinalBitmap,tempBitmap,(UI8)pWDP->width,(UI8)pWDP->height);
      pFinalBitmap = (ui8 *)tempBitmap;
      //D4W = (I16)(15 - (D4W&15));
  ...
  memmove( &LOCAL_22.b,pWDP, 4); // Temp copy so we can modify.
  if (drawingText)
  {
    i32 numLine;
    //LOCAL_4 = A3;  //RECTPOS
...
//    };
    ...
    if (numLine < 4)
    {
      //memmove( (ui8 *)&LOCAL_22,(ui8 *)A3, 8); //Why 8?
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  if (D6W == 3)
  {
    ...
    memmove( (ui8 *)&d.ViewportObjectButtons[5],(ui8 *)A3, 4);
  }
  else
  {
    D0W = sw(2*D7W + 102);
...
//
  memmove((ui8 *)tempBitmap, //temporary bitmap
	  (ui8 *)GetBasicGraphicAddress(graphicNum[doorType]),
          graphicSize);
  ...
  if (((D0W & 0x400)!=0) && (doorState!=5)) // Test most significant byte //'Lightning door' not bashed
  {
    if (STRandomBool())
    {
      Mirror(tempBitmap, rect[0].b.uByte4, rect[0].b.uByte5);
    };
    if (STRandomBool())
    {
      TAG008840(tempBitmap, rect[0].b.uByte4, rect[0].b.uByte5);
    };
  };
  //if (A3 == d.Byte3834)
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  else
  {
    ...
    A2 = (aReg)tempBitmap;
    memmove((ui8 *)A2,
	    (ui8 *)GetBasicGraphicAddress((uA3->b.y2 & 0x7f) + 69),
            D0W);
    if (uA3->b.y2 & 0x80)
    {
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  // patch area selected.
  pwA2[30] = 1;
  memmove(pwA2,TAG009110,42); // 21 words.  Up to the
              // trailing jump without the address
  // A3 points to first byte of function to be patched.
  memmove( pwA2+31,func, 6); // Save first 6 byes of func
...
//
         ...
         && ((*prnA2).IsSameObjectAs(objD7)) )
    {
      ...
      D0UW = uw(2*(D5W - D6W));
      memmove( (ui8 *)prnA2,(ui8 *)(prnA2+1), D0UW);
...
{//Wall text P3 = 0;
  ...
  DB2A1 = pText;
  DB2_12 = *DB2A1;
  A2 = (aReg)(d.compressedText + d.indirectTextIndex[DB2_12.index()]);
...
//
  ...
  else
  {
    if (P1 == 3)
    {
      memmove( (ui8 *)d.Byte20842,(ui8 *)d.Byte21418, 576);
      LOCAL_2[1] = 0;
      D6W = 8;
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  if (d.inStreamBuffered != 0)
  {
    ...
    memmove( dest,(ui8 *)d.inStreamNext, count);
...
{//(void)
  ...
  for (D7W = 1; D7W != 32; D7W++)
  {
    ...
    //memmove( (ui8 *)pntGear((pnt)A3+36),(ui8 *)pntGear((pnt)A3+32), 20608); // Dungeon interior.
    memmove( d.Pointer22916,d.Pointer22920, 20608); // Dungeon interior.
...
RESTARTABLE _GameSetup(i32 showPrisonDoor)
{
  ...
  //  memmove((pnt)&d.Palette11946,(pnt)palette, 32);
  //  memmove((pnt)&d.Palette11978,(pnt)palette, 32);
  TAG0023b0();
  TAG000ec6(); // In supervisor mode
  jdisint(5);  // TRAP 14 (#26,#5);
  //TAG0020ca();
  wvbl(_2_);
  memmove( (ui8 *)&d.Palette11946,(ui8 *)d.Palette552, 32); // Initialize palette
  memmove( (ui8 *)&d.Palette11978,(ui8 *)d.Palette552, 32); // Initialize palette
  if (showPrisonDoor == 1)
  {
    TAG01f746(_3_);
  }
...
/*
  ...
  TAG0020ca();
  wvbl(_2_);
  memmove( (pnt)&d.Palette11946,(pnt)d.Palette552, 32); // Initialize palette
  memmove( (pnt)&d.Palette11978,(pnt)d.Palette552, 32); // Initialize palette
  TAG01f746(_3_);
  */
