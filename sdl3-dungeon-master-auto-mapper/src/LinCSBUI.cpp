...
extern MIX_Mixer *sdl3_mixer;
extern SDL_AudioSpec audio_spec;
extern SDL_AudioSpec opening_doors_audio_spec;
...
*/
MIX_Audio *sound;
extern bool opening_doors;
static bool LIN_PlaySound(i8* audio, const ui32 size, int volume)
{
	if ( opening_doors)
	{
		sound= MIX_LoadRawAudio(sdl3_mixer,(void *) audio, size,&opening_doors_audio_spec);
	}
	else{
...
bool UI_PlaySound(const char *wave, uint32_t size, int32_t flags, int32_t attenuation /*not used*/)
{
//SDL_AudioSpec *SDL_LoadWAV(wave, SDL_AudioSpec *spec, Uint8 **audio_buf, Uint32 *audio_len);


	// la structure SNDHEAD contient le fichier wav en entier

	MIX_Audio *sound;
	
	if ( opening_doors)
	{
		sound= MIX_LoadRawAudio(sdl3_mixer,(void *) wave, size,&opening_doors_audio_spec);
	}
	else{
		sound= MIX_LoadRawAudio(sdl3_mixer,(void *) wave, size,&audio_spec);
	}
	return MIX_PlayAudio(sdl3_mixer,sound);
...
//SDL_AudioSpec *SDL_LoadWAV(const char *file, SDL_AudioSpec *spec, Uint8 **audio_buf, Uint32 *audio_len);
  return LIN_PlaySound((pnt)wave,size, 15);
}
